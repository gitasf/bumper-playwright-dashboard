...
- d6d25e7: Let a CI matrix that shards a suite without `--shard` declare its coordinates
...
- eccdb22: Strip a trailing slash from `WRIGHTFUL_URL` / the `url` option so a value like
